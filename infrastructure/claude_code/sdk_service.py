...
class ClaudeAgentSDKService:
    ...
    def __init__(
        ...
        # Set default working dir based on OS
        if default_working_dir is None:
            if os.name == 'nt':  # Windows
                default_working_dir = os.path.join(os.path.expanduser("~"), "projects")
            else:
    ...
    def _get_plugin_configs(self) -> list[dict]:
        ...
        for plugin_name in self.enabled_plugins:
            ...
            if plugin_name.startswith("official:"):
                ...
                logger.warning(f"Plugin '{name}' (official) cannot be passed to SDK plugins list. Skipping.")
                ...
                if os.path.isdir(plugin_path):
                    ...
                    logger.info(f"Plugin '{plugin_name}' (official) is handled globally. Not passing to SDK.")
        return plugins
    ...
    async def run_task(
        ...
        # Convert Linux path to Windows path if on Windows
        if os.name == 'nt' and work_dir.startswith('/root'):
            work_dir = work_dir.replace('/root', os.path.join(os.path.expanduser("~")))
            work_dir = work_dir.replace('/', os.sep)
            logger.info(f"[{user_id}] Converted Linux path to Windows: {work_dir}")
        ...
        try:
            ...
            if plugins:
                logger.info(f"[{user_id}] Using {len(plugins)} plugins: {[p.get('path', p.get('name')) for p in plugins]}")
            ...
            # Build options
            # Initialize buffer for stderr
            stderr_output = []
            def stderr_callback(line: str):
                stderr_output.append(line)
                if "Error" in line or "Exception" in line or "Traceback" in line:
                    logger.warning(f"[{user_id}] CLI STDERR: {line}")
                    ...
                    if isinstance(message, AssistantMessage):
                        ...
                        for block in message.content:
                            if isinstance(block, TextBlock):
                                text = block.text
                                # Safe logging for Windows console compatibility
                                safe_text = text[:100].encode('utf-8', errors='ignore').decode('utf-8')
                                logger.info(f"[{user_id}] TextBlock: {safe_text}...")
            ...
            # Check for captured stderr which might explain the crash
            if stderr_output:
                stderr_content = "\n".join(stderr_output[-20:])  # Last 20 lines
                error_msg += f"\n\nCLI STDERR:\n{stderr_content}"
                logger.error(f"[{user_id}] CLI Crashed with STDERR:\n{stderr_content}")
            ...
            # Auto-recovery for resume failures
            # If we were trying to resume a session and it crashed, try starting a fresh session
            if session_id and not _retry_without_resume:
                logger.warning(
                    f"[{user_id}] Task crashed while resuming session {session_id[:8]}... "
                    f"Retrying with new session... Error: {error_msg}"
                )
