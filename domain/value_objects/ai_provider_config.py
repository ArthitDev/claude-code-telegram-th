...
class AIModelConfig:
    ...
    opus: str
    default: str
    context_window: int = 200_000  # Default context window (tokens)
...
class AIProviderConfig:
    ...
    def _get_default_model_config(self) -> AIModelConfig:
        ...
        if self.provider_type == AIProviderType.ZHIPU_AI:
            return AIModelConfig(
                ...
                context_window=200_000,  # GLM-4.7 has 200K context window
            )
        if self.provider_type == AIProviderType.MOONSHOT_KIMI:
            return AIModelConfig(
                haiku="kimi-2.5",
                sonnet="kimi-2.5",
                opus="kimi-2.5",
                default="kimi-2.5",
                context_window=256_000,  # Kimi 2.5 has 256K context window
            )
        ...
        return AIModelConfig(
            ...
            context_window=200_000,  # Claude 3.5 Sonnet context window
        )
    ...
    def from_env(
        ...
        if base_url:
            ...
            elif "kimi" in base_url.lower():
                provider_type = AIProviderType.MOONSHOT_KIMI
            else:
    ...
    def context_window(self) -> int:
        """Get the context window size for this provider's model"""
        return (
            self.model_config.context_window
            if self.model_config
            else 200_000  # Default fallback
        )
