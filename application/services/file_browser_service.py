...
class FileBrowserService:
    ...
    def _build_tree_html(
        ...
        if len(content.entries) >= self.MAX_ENTRIES:
            lines.append(f"<i>... and more files (showing {self.MAX_ENTRIES})</i>")

        # Add invisible timestamp to prevent "message not modified" error
        import time
        timestamp = int(time.time() * 1000) % 1000000
        lines.append(f"<code> </code>")  # Invisible space to make message unique
