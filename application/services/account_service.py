...
class ClaudeModel(str, Enum):
    ...
    def get_description(cls, model: str) -> str:
        ...
        descriptions = {
            cls.OPUS: "โมเดลที่ทรงพลังที่สุด เหมาะสำหรับงานที่ซับซ้อน",
            cls.SONNET: "สมดุลระหว่างความเร็วและคุณภาพ (แนะนำ)",
            cls.HAIKU: "โมเดลความเร็วสูงสำหรับงานง่ายๆ",
        }
...
class ZaiConfig:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "ZaiConfig":
        return cls(
            api_key=data.get("api_key", ""),
            base_url=data.get("base_url", "https://open.bigmodel.cn/api/anthropic"),
...
class KimiConfig:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "KimiConfig":
        return cls(
            api_key=data.get("api_key", ""),
            base_url=data.get("base_url", "https://api.kimi.com/coding/"),
            model=data.get("model", "kimi-k2-0711-preview"),
        )
...
class AccountSettings:
    ...
    user_id: int
    auth_mode: AuthMode = AuthMode.MOONSHOT_KIMI
    ...
    zai_api_key: Optional[str] = None  # User-provided z.ai API key (legacy, use zai_config instead)
    zai_config: Optional[ZaiConfig] = None  # z.ai API configuration
    kimi_config: Optional[KimiConfig] = None  # Moonshot Kimi configuration
    language: str = "ru"  # Language preference (ru, en, zh)
    os_type: str = "auto"  # OS type: windows, linux, linux_user, macos, auto
    custom_working_dir: Optional[str] = None  # Custom working directory path
...
class AccountService:
    ...
    async def get_settings(self, user_id: int) -> AccountSettings:
        ...
        if not settings:
            settings = AccountSettings(
                user_id=user_id,
                auth_mode=AuthMode.MOONSHOT_KIMI,
    ...
    async def set_auth_mode(self, user_id: int, mode: AuthMode) -> tuple[bool, AccountSettings, Optional[str]]:
        ...
        if mode == AuthMode.CLAUDE_ACCOUNT:
            if not self.has_valid_credentials():
                ...
                settings = await self.get_settings(user_id)
                return False, settings, "❌ ไม่พบไฟล์ข้อมูลรับรองหรือโทเค็นหมดอายุ กรุณาอัปโหลด credentials.json หรือเข้าสู่ระบบผ่าน OAuth"
    ...
    async def get_account_settings(self, user_id: int) -> Optional["AccountSettings"]:
        """
        Get full account settings for a user.

        Args:
            user_id: User ID (int)

        Returns:
            AccountSettings or None if not found
        """
        return await self.repository.find_by_user_id(user_id)

    async def set_os_type(self, user_id: int, os_type: str) -> None:
        """
        Set user's OS type preference.

        Args:
            user_id: User ID (int)
            os_type: OS type (windows, linux, linux_user, macos, auto)
        """
        valid_os_types = ["windows", "linux", "linux_user", "macos", "auto"]
        ...
        if settings:
            ...
            # Create new settings with OS type
            settings = AccountSettings(
                user_id=user_id,
                os_type=os_type,
                auth_mode=AuthMode.MOONSHOT_KIMI,
        ...
        logger.info(f"[{user_id}] OS type set to: {os_type}")
    ...
    async def get_available_models(self, user_id: int) -> list[dict]:
        ...
        if settings.auth_mode == AuthMode.CLAUDE_ACCOUNT:
            models = [
                {"id": ClaudeModel.OPUS.value, "name": "Opus 4.5", "desc": "โมเดลที่ทรงพลังที่สุด"},
                {"id": ClaudeModel.SONNET.value, "name": "Sonnet 4.5", "desc": "สมดุลความเร็วและคุณภาพ (แนะนำ)"},
                {"id": ClaudeModel.HAIKU.value, "name": "Haiku 4", "desc": "โมเดลความเร็วสูง"},
            ]
            for m in models:
        ...
        elif settings.auth_mode == AuthMode.MOONSHOT_KIMI:
            if settings.kimi_config:
                return [{
                    "id": settings.kimi_config.model,
                    "name": self._format_model_name(settings.kimi_config.model),
                    "desc": settings.kimi_config.base_url,
                    "is_selected": True,
                }]
            # Fallback to default Kimi model
            return [{
                "id": "kimi-k2-0711-preview",
                "name": "Kimi K2",
                "desc": "Moonshot Kimi Code API",
                "is_selected": True,
            }]
    ...
    def _get_zai_models_from_env(self) -> list[dict]:
        ...
        if haiku:
            models.append({"id": haiku, "name": self._format_model_name(haiku), "desc": "โมเดลความเร็วสูง"})
        if sonnet and sonnet != haiku:
            models.append({"id": sonnet, "name": self._format_model_name(sonnet), "desc": "โมเดลสมดุล"})
        if opus and opus not in (haiku, sonnet):
            models.append({"id": opus, "name": self._format_model_name(opus), "desc": "โมเดลทรงพลัง"})
        if default and default not in (haiku, sonnet, opus):
            models.append({"id": default, "name": self._format_model_name(default), "desc": "โมเดลเริ่มต้น"})

        # Fallback to hardcoded z.ai defaults
        if not models:
            models = [
                {"id": "glm-4.5-air", "name": "GLM 4.5 Air", "desc": "โมเดลความเร็วสูง"},
                {"id": "glm-4.7", "name": "GLM 4.7", "desc": "โมเดลทรงพลัง"},
            ]
    ...
    async def set_zai_api_key(
        ...
        return True, "✅ บันทึกและตรวจสอบ API Key z.ai เรียบร้อย!", settings
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    return False, "❌ API Key ไม่ถูกต้อง (401 Unauthorized)"
                elif response.status_code == 403:
                    return False, "❌ ไม่อนุญาตให้เข้าถึง (403 Forbidden) โปรดตรวจสอบสิทธิ์ของคีย์"
            ...
            return False, "❌ หมดเวลาการเชื่อมต่อขณะตรวจสอบคีย์ ลองใหม่อีกครั้ง"
        except httpx.ConnectError:
            return False, "❌ ไม่สามารถเชื่อมต่อกับ z.ai API ได้ โปรดตรวจสอบอินเทอร์เน็ต"
        except Exception as e:
            logger.error(f"Error validating z.ai API key: {e}")
            return False, f"❌ เกิดข้อผิดพลาดในการตรวจสอบ: {str(e)}"
    ...
    def get_env_for_mode(
        ...
        zai_config: Optional[ZaiConfig] = None,
        ...
        Args:
            ...
            zai_api_key: User-provided z.ai API key (legacy, use zai_config instead)
            zai_config: z.ai API configuration (required for ZAI_API mode)
            proxy_config: Pre-fetched proxy configuration (from async ProxyService)
            kimi_config: Moonshot Kimi configuration (required for MOONSHOT_KIMI mode)
            user_id: User ID for fetching settings if needed
        ...
        if mode == AuthMode.ZAI_API:
            # z.ai API mode - use zai_config if available, fallback to legacy zai_api_key or env vars
            if zai_config:
                # New: Use full zai_config
                env["ANTHROPIC_BASE_URL"] = zai_config.base_url
                env["ANTHROPIC_API_KEY"] = zai_config.api_key
                env["ANTHROPIC_MODEL"] = zai_config.model
                logger.debug(f"z.ai mode env (from zai_config): base_url={zai_config.base_url}, model={zai_config.model}")
            else:
                # Legacy: Use individual key or env vars
                ...
                logger.debug(f"z.ai mode env (legacy): base_url={base_url is not None}, model={model}, user_key={zai_api_key is not None}")
        ...
        elif mode == AuthMode.MOONSHOT_KIMI:
            # Moonshot Kimi mode - use user-provided config
            # Get kimi_config from parameter or repository
            if not kimi_config and user_id:
                # Try to get from user settings
                import asyncio
                try:
                    settings = asyncio.get_event_loop().run_until_complete(
                        self.get_settings(user_id)
                    )
                    kimi_config = settings.kimi_config
                except Exception:
                    pass

            if kimi_config:
                env["ANTHROPIC_BASE_URL"] = kimi_config.base_url
                env["ANTHROPIC_API_KEY"] = kimi_config.api_key
                env["ANTHROPIC_MODEL"] = kimi_config.model
                logger.debug(f"Moonshot Kimi mode: base_url={kimi_config.base_url}, model={kimi_config.model}")
            else:
                ...
                model = os.environ.get("ANTHROPIC_MODEL", "kimi-k2-0711-preview")
                if base_url:
                    env["ANTHROPIC_BASE_URL"] = base_url
                if api_key:
                ...
                if model:
                    env["ANTHROPIC_MODEL"] = model
                logger.debug(f"Moonshot Kimi mode (env fallback): base_url={base_url is not None}, model={model}")
    ...
    def apply_env_for_mode(
        ...
        # Fetch zai_config from settings if user_id provided and mode is ZAI_API
        if mode == AuthMode.ZAI_API and user_id and not zai_config:
            try:
                import asyncio
                settings = asyncio.get_event_loop().run_until_complete(
                    self.get_settings(user_id)
                )
                zai_config = settings.zai_config
            except Exception:
                pass

        mode_env = self.get_env_for_mode(
            mode, local_config=local_config, zai_api_key=zai_api_key,
            zai_config=zai_config, proxy_config=proxy_config, kimi_config=kimi_config, user_id=user_id
        )
    ...
    def delete_credentials(self) -> tuple[bool, str]:
        ...
        try:
            if os.path.exists(CREDENTIALS_PATH):
                ...
                return True, "✅ ลบไฟล์ credentials.json แล้ว"
            else:
                return False, "❌ ไม่พบไฟล์ credentials.json"
        except Exception as e:
            logger.error(f"Error deleting credentials: {e}")
            return False, f"❌ เกิดข้อผิดพลาดในการลบ: {str(e)}"

    # ============== z.ai API Configuration ==============

    async def has_zai_config(self, user_id: int) -> bool:
        """Check if user has z.ai API configuration"""
        settings = await self.get_settings(user_id)
        return settings.zai_config is not None and bool(settings.zai_config.api_key)

    async def set_zai_config(
        self, user_id: int, api_key: str, base_url: str = "https://open.bigmodel.cn/api/anthropic", model: str = "glm-4.7"
    ) -> tuple[bool, str, Optional[AccountSettings]]:
        """
        Set z.ai API configuration for user.

        Args:
            user_id: User ID
            api_key: z.ai API key
            base_url: z.ai API base URL (default: https://open.bigmodel.cn/api/anthropic)
        ...
        if not api_key or len(api_key) < 10:
            return False, "❌ API Key ไม่ถูกต้อง (สั้นเกินไป)", None

        # Validate API key with test request
        is_valid, error_msg = await self._validate_zai_api_key(api_key, base_url)
        if not is_valid:
            return False, f"❌ {error_msg}", None

        # Save configuration
        settings = await self.get_settings(user_id)
        settings.zai_config = ZaiConfig(
            api_key=api_key,
            base_url=base_url.rstrip("/"),
            model=model,
        )
        # Also update legacy zai_api_key for backward compatibility
        settings.zai_api_key = api_key
        settings.updated_at = datetime.now()

        await self.repository.save(settings)
        logger.info(f"Saved z.ai config for user {user_id}: base_url={base_url}, model={model}")

        return True, f"✅ บันทึก z.ai API Key สำเร็จ\n🤖 Model: {model}", settings

    async def _validate_zai_api_key(self, api_key: str, base_url: str = "https://open.bigmodel.cn/api/anthropic") -> tuple[bool, str]:
        """Validate z.ai API key by making a test request."""
        import httpx
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    return False, "API Key ไม่มีสิทธิ์ใช้งาน (403 Forbidden)"
                else:
                    return False, f"API ตอบกลับด้วยรหัส {response.status_code}"
        except httpx.TimeoutException:
            return False, "หมดเวลาเชื่อมต่อ (timeout)"
        except Exception as e:
            logger.error(f"Error validating z.ai API key: {e}")
            return False, f"ไม่สามารถเชื่อมต่อได้: {str(e)}"

    async def get_zai_config(self, user_id: int) -> Optional[ZaiConfig]:
        """Get z.ai API configuration for user"""
        settings = await self.get_settings(user_id)
        return settings.zai_config

    async def clear_zai_config(self, user_id: int) -> tuple[bool, str]:
        """Clear z.ai API configuration for user"""
        settings = await self.get_settings(user_id)
        settings.zai_config = None
        settings.zai_api_key = None
        settings.updated_at = datetime.now()
        await self.repository.save(settings)
        logger.info(f"Cleared z.ai config for user {user_id}")
        return True, "✅ ลบการตั้งค่า z.ai แล้ว"

    # ============== Moonshot Kimi Configuration ==============

    async def has_kimi_config(self, user_id: int) -> bool:
        """Check if user has Moonshot Kimi configuration"""
        settings = await self.get_settings(user_id)
        return settings.kimi_config is not None and bool(settings.kimi_config.api_key)

    async def set_kimi_config(
        self, user_id: int, api_key: str, base_url: str = "https://api.kimi.com/coding/", model: str = "kimi-k2-0711-preview"
    ) -> tuple[bool, str, Optional[AccountSettings]]:
        """
        Set Moonshot Kimi configuration for user.

        Args:
            user_id: User ID
            api_key: Kimi API key
            base_url: Kimi API base URL (default: https://api.kimi.com/coding/)
            model: Model name (default: kimi-k2-0711-preview)
        ...
        # Validate API key with test request
        is_valid, error_msg = await self._validate_kimi_api_key(api_key, base_url)
        if not is_valid:
            return False, f"❌ {error_msg}", None

        # Save configuration
        settings = await self.get_settings(user_id)
        settings.kimi_config = KimiConfig(
            api_key=api_key,
            base_url=base_url.rstrip("/"),
            model=model,
        )
        ...
        await self.repository.save(settings)
        logger.info(f"Saved Kimi config for user {user_id}: base_url={base_url}, model={model}")

        return True, f"✅ บันทึก Kimi API Key สำเร็จ\n🤖 Model: {model}", settings

    async def _validate_kimi_api_key(self, api_key: str, base_url: str = "https://api.kimi.com/coding/") -> tuple[bool, str]:
        """Validate Kimi API key by making a test request."""
        import httpx
        ...
        try:
            ...
            logger.error(f"Error validating Kimi API key: {e}")
            return False, f"ไม่สามารถเชื่อมต่อได้: {str(e)}"

    async def get_kimi_config(self, user_id: int) -> Optional[KimiConfig]:
        """Get Moonshot Kimi configuration for user"""
        settings = await self.get_settings(user_id)
        return settings.kimi_config

    async def clear_kimi_config(self, user_id: int) -> tuple[bool, str]:
        """Clear Moonshot Kimi configuration for user"""
        settings = await self.get_settings(user_id)
        settings.kimi_config = None
        settings.updated_at = datetime.now()
        await self.repository.save(settings)
        logger.info(f"Cleared Kimi config for user {user_id}")
        return True, "✅ ลบการตั้งค่า Kimi แล้ว"

    async def reset_to_env_defaults(self, user_id: int) -> tuple[bool, str, AccountSettings]:
        """
        Reset user configuration to use environment variable defaults (.env).

        This clears all user-specific API configurations (zai_config, kimi_config, zai_api_key)
        and resets auth_mode to default from environment.
        ...
        # Clear all user-specific API configurations
        settings.zai_config = None
        settings.zai_api_key = None
        settings.kimi_config = None
        settings.local_model_config = None

        # Reset auth_mode to default from environment or MOONSHOT_KIMI
        import os
        default_mode = os.environ.get("DEFAULT_AUTH_MODE", "moonshot_kimi")
        try:
            settings.auth_mode = AuthMode(default_mode)
        except ValueError:
            settings.auth_mode = AuthMode.MOONSHOT_KIMI

        # Clear model preference to use env default
        settings.model = None

        settings.updated_at = datetime.now()
        await self.repository.save(settings)

        logger.info(f"[{user_id}] Reset to environment defaults: auth_mode={settings.auth_mode.value}")

        # Get current env values for display
        env_provider = "Moonshot Kimi" if settings.auth_mode == AuthMode.MOONSHOT_KIMI else \
                      "z.ai" if settings.auth_mode == AuthMode.ZAI_API else \
                      "Claude Account" if settings.auth_mode == AuthMode.CLAUDE_ACCOUNT else \
                      "Local Model"

        return True, f"✅ รีเซ็ตเป็นค่าเริ่มต้นแล้ว\n🔄 โหมด: {env_provider}\n📄 ใช้ค่าจาก .env", settings
