    ...
    async def handle_text(
        ...
        if self.project_service and self.context_service:
            try:
                ...
                project = await self.project_service.get_current(uid)
                if project:
                    # Use user_state working_dir if it differs from project (user may have changed OS)
                    user_working_dir = self.get_working_dir(user_id)
                    if user_working_dir and user_working_dir != project.working_dir:
                        # Update project working_dir to match user_state
                        await self.project_service.update_working_dir(uid, user_working_dir)
                        working_dir = user_working_dir
                        logger.info(f"[{user_id}] Updated project working_dir to match user_state: {working_dir}")
                    else:
        ...
        streaming = StreamingHandler(bot, message.chat.id, reply_markup=cancel_keyboard, translator=t)
