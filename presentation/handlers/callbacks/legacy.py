...
from presentation.handlers.callbacks.workspace import WorkspaceCallbackHandler
...
class CallbackHandlers:
    ...
    - WorkspaceCallbackHandler: workspace_*
    """
    ...
    def __init__(
        ...
        # Initialize workspace handler with workspace_service
        workspace_args = (
            bot_service, message_handlers, claude_proxy, sdk_service,
            project_service, context_service, file_browser_service
        )
        self._workspace = WorkspaceCallbackHandler(*workspace_args)
        self._workspace.workspace_service = workspace_service
    ...
    async def handle_command_approve(self, callback: CallbackQuery) -> None:
        ...
        if not command_id:
            await safe_callback_answer(callback, "❌ Неверная команда")
            return
    ...
    # ============== Workspace Management Callbacks (delegated to _workspace) ==============

    async def handle_workspace_menu(self, callback: CallbackQuery) -> None:
        await self._workspace.handle_workspace_menu(callback)

    async def handle_workspace_switch(self, callback: CallbackQuery) -> None:
        await self._workspace.handle_workspace_switch(callback)

    async def handle_workspace_browse(self, callback: CallbackQuery) -> None:
        await self._workspace.handle_workspace_browse(callback)

    async def handle_workspace_nav(self, callback: CallbackQuery) -> None:
        await self._workspace.handle_workspace_nav(callback)

    async def handle_workspace_page(self, callback: CallbackQuery) -> None:
        await self._workspace.handle_workspace_page(callback)

    async def handle_workspace_noop(self, callback: CallbackQuery) -> None:
        """Handle no-op button (page indicator)."""
        await safe_callback_answer(callback)

    async def handle_workspace_drives(self, callback: CallbackQuery) -> None:
        await self._workspace.handle_workspace_drives(callback)

    async def handle_workspace_add(self, callback: CallbackQuery) -> None:
        await self._workspace.handle_workspace_add(callback)

    async def handle_workspace_remove(self, callback: CallbackQuery) -> None:
        await self._workspace.handle_workspace_remove(callback)

    # ============== OS Selection Callbacks ==============

    async def handle_os_select(self, callback: CallbackQuery) -> None:
        """Handle OS selection callback."""
        # Parse: os:select:<os_type>
        parts = callback.data.split(":")
        os_type = parts[2] if len(parts) > 2 else "auto"
        user_id = callback.from_user.id

        # Save OS type to account settings
        if self.account_service:
            try:
                await self.account_service.set_os_type(user_id, os_type)
            except Exception as e:
                logger.error(f"Error saving OS type: {e}")

        # Get working directory for selected OS
        working_dir = self._get_working_dir_for_os(os_type)

        # Update user_state working directory
        if self.message_handlers:
            self.message_handlers.set_working_dir(user_id, working_dir)

        # Show confirmation and continue to main menu
        os_names = {
            "windows": "🪟 Windows",
            "linux": "🐧 Linux (Root)",
        ...
        os_name = os_names.get(os_type, os_type)

        await callback.message.edit_text(
            f"✅ <b>OS Selected: {os_name}</b>\n\n"
            f"Working Directory: <code>{working_dir}</code>\n\n"
            f"Press ▶️ Start to continue...",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="▶️ Start", callback_data="menu:main")]
            ])
        )
        await safe_callback_answer(callback, f"✅ {os_name}")

    def _get_working_dir_for_os(self, os_type: str) -> str:
        """Get working directory for selected OS."""
        import getpass
...
def register_handlers(router: Router, handlers: CallbackHandlers) -> None:
    ...
    router.callback_query.register(
        handlers.handle_workspace_menu,
        F.data == "workspace:menu"
    )
    router.callback_query.register(
        handlers.handle_workspace_switch,
        F.data.startswith("workspace:switch:")
    )
    router.callback_query.register(
        handlers.handle_workspace_browse,
        F.data == "workspace:browse"
    )
    router.callback_query.register(
        handlers.handle_workspace_nav,
        F.data.startswith("workspace:nav:")
    )
    router.callback_query.register(
        handlers.handle_workspace_page,
        F.data.startswith("workspace:page:")
    )
    router.callback_query.register(
        handlers.handle_workspace_drives,
        F.data == "workspace:drives"
    )
    router.callback_query.register(
        handlers.handle_workspace_add,
        F.data.startswith("workspace:add:")
    )
    router.callback_query.register(
        handlers.handle_workspace_noop,
        F.data == "workspace:noop"
    )
    router.callback_query.register(
        handlers.handle_workspace_remove,
        F.data.startswith("workspace:remove:")
    )

    # OS Selection handlers
    router.callback_query.register(
        handlers.handle_os_select,
        F.data.startswith("os:select:")
    )
