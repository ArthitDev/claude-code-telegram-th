...
class AccountStates(StatesGroup):
    ...
    # Moonshot Kimi setup states
    waiting_kimi_api_key = State()
    waiting_kimi_base_url = State()
...
class AccountHandlers:
    ...
    def _register_handlers(self):
        ...
        # Moonshot Kimi setup handlers
        self.router.message.register(
            self.handle_kimi_api_key_input,
            AccountStates.waiting_kimi_api_key,
            F.text
        )
        self.router.message.register(
            self.handle_kimi_base_url_input,
            AccountStates.waiting_kimi_base_url,
            F.text
        )
    ...
    async def handle_account_command(self, message: Message, state: FSMContext):
        ...
        mode_names = {
            ...
            AuthMode.MOONSHOT_KIMI: "Moonshot Kimi",
        ...
        if settings.auth_mode == AuthMode.ZAI_API:
            ...
            has_kimi = bool(settings.kimi_config and settings.kimi_config.api_key)
            key_status = "✅" if has_kimi else "❌"
            text += f"🔑 Kimi API key: {key_status}\n"
            if has_kimi and settings.kimi_config.model:
                text += f"🤖 Model: {settings.kimi_config.model}\n"
    ...
    async def handle_account_callback(self, callback: CallbackQuery, state: FSMContext):
        ...
        if action == "mode":
            ...
            await state.clear()
            await self._show_menu(callback, state)
            await safe_callback_answer(callback, t("account.upload_cancelled"))
        ...
        elif action == "kimi_setup":
            # Start Moonshot Kimi setup
            await self._start_kimi_setup(callback, state)

        elif action == "kimi_delete":
            # Delete Moonshot Kimi config
            await self._handle_kimi_delete_config(callback, state)

        elif action == "kimi_setup_url":
            # Start Moonshot Kimi setup with custom URL
            await self._start_kimi_url_setup(callback, state)

        elif action == "reset_to_env":
            # Reset to environment defaults
            await self._handle_reset_to_env(callback, state)

        elif action == "confirm_reset_env":
            # Confirm reset to environment defaults
            await self._confirm_reset_to_env(callback, state)

        else:
            lang = await self._get_user_lang(callback.from_user.id)
            from shared.i18n import get_translator
            t = get_translator(lang)
            await safe_callback_answer(callback, t("account.action_unknown", action=action))
    ...
    async def _handle_mode_selection(
        ...
        try:
            ...
            from shared.i18n import get_translator
            t = get_translator(lang)
            await safe_callback_answer(callback, t("account.mode_unknown", mode=mode_str))
            return
        ...
        if settings.auth_mode == mode:
            if mode == AuthMode.CLAUDE_ACCOUNT:
                ...
                # Show Moonshot Kimi submenu with key management
                await self._show_kimi_submenu(callback, state)
                return
            else:
                lang = await self._get_user_lang(user_id)
                from shared.i18n import get_translator
                t = get_translator(lang)
                await safe_callback_answer(callback, t("account.mode_already_selected"))
                return
        ...
        # For Moonshot Kimi, check if user has config
        if mode == AuthMode.MOONSHOT_KIMI:
            has_kimi = await self.account_service.has_kimi_config(user_id)
            if not has_kimi:
                # No config - ask user to set up
                text = (
                    f"🌙 <b>Moonshot Kimi Setup</b>\n\n"
                    f"Moonshot Kimi Code API สำหรับ Claude Code\n\n"
                    f"ต้องการตั้งค่า API Key ก่อนใช้งาน:\n"
                    f"1. ไปที่ https://platform.moonshot.cn\n"
                    f"2. สร้าง API Key\n"
                    f"3. กรอก API Key ในเมนูต่อไป\n\n"
                    f"<i>หรือใช้ Environment Variables ที่ตั้งไว้</i>"
                )

                await callback.message.edit_text(
                    text,
                    reply_markup=Keyboards.kimi_auth_options(lang=lang),
                    parse_mode="HTML",
        ...
        if mode == AuthMode.CLAUDE_ACCOUNT:
            ...
            # Moonshot Kimi with config
            text = (
                f"🌙 <b>Switch to Moonshot Kimi</b>\n\n"
                f"คุณกำลังจะเปลี่ยนเป็นโหมด Moonshot Kimi\n"
                f"Claude Code จะใช้ Kimi API แทน Claude API\n\n"
                f"<i>หมายเหตุ: ต้องตั้งค่า API Key ก่อนใช้งาน</i>"
            )
        else:
            # z.ai with key
            text = (
    ...
    async def _handle_mode_confirm(
        ...
        try:
            ...
            await safe_callback_answer(callback, t("account.unknown_mode", mode=mode_str))
            return
        ...
        if not success:
            ...
            await safe_callback_answer(callback, t("account.switch_error", error=error_msg), show_alert=True)
        ...
        await safe_callback_answer(callback, t("account.switch_success", mode=mode_name))
    ...
    async def _show_kimi_submenu(self, callback: CallbackQuery, state: FSMContext):
        """Show Moonshot Kimi submenu with key management options"""
        user_id = callback.from_user.id
        lang = await self._get_user_lang(user_id)

        settings = await self.account_service.get_settings(user_id)
        has_config = bool(settings.kimi_config and settings.kimi_config.api_key)

        text = "🌙 <b>Moonshot Kimi</b>\n\n"

        if has_config:
            # Mask the key for display (show first 8 and last 4 chars)
            key = settings.kimi_config.api_key
            if len(key) > 16:
                masked = f"{key[:8]}...{key[-4:]}"
            else:
                masked = f"{key[:4]}***"
            text += f"Status: ✅ API key configured\n"
            text += f"Key: <code>{masked}</code>\n"
            text += f"Base URL: <code>{settings.kimi_config.base_url}</code>\n"
            text += f"Model: <code>{settings.kimi_config.model}</code>\n"
        else:
            text += "Status: ❌ API key not configured\n"
            text += "\n<i>ใช้ Environment Variables หรือตั้งค่า API Key</i>"

        await callback.message.edit_text(
            text,
            reply_markup=Keyboards.kimi_api_submenu(
                has_config=has_config,
                current_model=settings.kimi_config.model if has_config else None,
                lang=lang
            ),
            parse_mode="HTML"
        )
    ...
    async def _handle_model_selection(
        ...
        await safe_callback_answer(callback, msg, show_alert=session_reset)
    ...
    async def _handle_delete_account(self, callback: CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            if success:
                ...
                # Return to menu
                await self._show_menu(callback, state)
                await safe_callback_answer(callback, t("account.logged_out"))
            else:
                await safe_callback_answer(callback, message, show_alert=True)

        except Exception as e:
            logger.error(f"[{user_id}] Error deleting account: {e}", exc_info=True)
            await safe_callback_answer(callback, t("account.upload_error", error=str(e)), show_alert=True)
    ...
    async def handle_cancel_upload_text(self, message: Message, state: FSMContext):
        ...
        cancel_words = ["cancel", "/cancel", t("cancel_words")]
        if message.text and message.text.lower() in cancel_words:
            await state.clear()
    ...
    async def _handle_login(self, callback: CallbackQuery, state: FSMContext):
        ...
        from shared.i18n import get_translator
        t = get_translator(lang)
        await safe_callback_answer(callback, t("status.loading"))
    ...
    async def handle_oauth_code_input(self, message: Message, state: FSMContext):
        ...
        if success:
            ...
            # Parse error message
            if result_msg.startswith("oauth_error_save|"):
                error_output = result_msg.split("|", 1)[1]
                error_text = t("account.oauth_error_save", output=error_output)
            elif result_msg.startswith("oauth_error|"):
                error_str = result_msg.split("|", 1)[1]
                error_text = t("account.oauth_error", error=error_str)
            else:
    ...
    async def _cancel_oauth_login(self, callback: CallbackQuery, state: FSMContext):
        ...
        await state.clear()
        await self._show_menu(callback, state)
        await safe_callback_answer(callback, t("account.oauth_cancelled"))
    ...
    async def _complete_local_model_setup(
        ...
        text = (
            ...
            f"{t('account.local_model_display', model=model_name)}\n\n"
            f"{t('account.confirm_switch_claude_note')}"
        )
    ...
    async def handle_zai_api_key_input(self, message: Message, state: FSMContext):
        ...
        # Check for cancel commands
        cancel_words = ["cancel", "/cancel", t("cancel_words")]
        if api_key.lower() in cancel_words:
            await state.clear()
    ...
    async def _handle_zai_delete_key(self, callback: CallbackQuery, state: FSMContext):
        ...
        if success:
            await self._show_menu(callback, state)
            await safe_callback_answer(callback, t("account.zai_key_deleted"))
        else:
            await safe_callback_answer(callback, message, show_alert=True)
    ...
    async def _start_kimi_url_setup(self, callback: CallbackQuery, state: FSMContext):
        """Start Moonshot Kimi setup flow with custom URL - ask for base URL first"""
        user_id = callback.from_user.id
        lang = await self._get_user_lang(user_id)

        text = (
            "🌙 <b>Moonshot Kimi Setup</b>\n\n"
            "กรุณาส่ง Base URL สำหรับ Kimi API:\n\n"
            "<b>ค่าเริ่มต้น:</b>\n"
            "• <code>https://api.kimi.com/coding/</code>\n\n"
            "<b>หรือ URL อื่นที่รองรับ Anthropic-compatible API</b>\n\n"
            "<i>พิมพ์ 'cancel' เพื่อยกเลิก</i>"
        )

        await state.set_state(AccountStates.waiting_kimi_base_url)
        await callback.message.edit_text(
            text,
            reply_markup=Keyboards.cancel_only(lang=lang),
            parse_mode="HTML"
    ...
    async def _start_kimi_setup(self, callback: CallbackQuery, state: FSMContext):
        """Start Moonshot Kimi setup flow - ask for API key"""
        user_id = callback.from_user.id
        lang = await self._get_user_lang(user_id)

        # Check if user already has config
        has_config = await self.account_service.has_kimi_config(user_id)

        text = "🌙 <b>Moonshot Kimi Setup</b>\n\n"

        if has_config:
            settings = await self.account_service.get_settings(user_id)
            text += f"✅ <i>API Key ที่บันทึกไว้:</i> <code>{settings.kimi_config.api_key[:8]}...{settings.kimi_config.api_key[-4:]}</code>\n\n"

        text += (
            "กรุณาส่ง API Key สำหรับ Moonshot Kimi:\n\n"
            "<b>วิธีได้รับ API Key:</b>\n"
            "1. ไปที่ https://platform.moonshot.cn\n"
            "2. สร้าง API Key\n"
            "3. คัดลอกและส่งมาที่นี่\n\n"
            "<i>หรือพิมพ์ 'cancel' เพื่อยกเลิก</i>"
        )

        await state.set_state(AccountStates.waiting_kimi_api_key)
        await callback.message.edit_text(
            text,
            reply_markup=Keyboards.kimi_api_key_input(has_existing_key=has_config, lang=lang),
            parse_mode="HTML",
            disable_web_page_preview=True
        )
        await callback.answer()

    async def handle_kimi_api_key_input(self, message: Message, state: FSMContext):
        """Handle Moonshot Kimi API key input from user"""
        user_id = message.from_user.id
        lang = await self._get_user_lang(user_id)
        from shared.i18n import get_translator
        t = get_translator(lang)
        ...
        if api_key.lower() in cancel_words:
            await state.clear()
            await message.answer("❌ ยกเลิกการตั้งค่า Moonshot Kimi")
            return
        ...
        # Get base URL from state or use default
        state_data = await state.get_data()
        base_url = state_data.get("kimi_base_url", "https://api.kimi.com/coding/")

        # Validate and save the key
        success, result_msg, settings = await self.account_service.set_kimi_config(
            user_id, api_key, base_url=base_url
        )
        ...
        if success:
            ...
            await processing_msg.edit_text(
                f"✅ บันทึก Moonshot Kimi API Key สำเร็จ!\n\n"
                f"คุณสามารถเปลี่ยนเป็นโหมด Moonshot Kimi ได้จากเมนู Account",
                reply_markup=Keyboards.account_menu(
                    current_mode=settings.auth_mode.value,
                    has_credentials=creds_info.exists,
                    subscription_type=creds_info.subscription_type,
            ...
            logger.info(f"[{user_id}] Moonshot Kimi API key saved successfully")
        else:
            # Key is invalid
            await processing_msg.edit_text(
                f"❌ ไม่สามารถบันทึก API Key ได้:\n{result_msg}\n\n"
                f"กรุณาลองใหม่อีกครั้ง",
                reply_markup=Keyboards.kimi_api_key_input(has_existing_key=False, lang=lang),
                parse_mode="HTML"
            )

    async def handle_kimi_base_url_input(self, message: Message, state: FSMContext):
        """Handle Moonshot Kimi base URL input from user"""
        user_id = message.from_user.id
        lang = await self._get_user_lang(user_id)
        from shared.i18n import get_translator
        t = get_translator(lang)
        ...
        # Check for cancel commands
        cancel_words = ["cancel", "/cancel", t("cancel_words")]
        if base_url.lower() in cancel_words:
            await state.clear()
            await message.answer("❌ ยกเลิกการตั้งค่า Moonshot Kimi")
            return
        ...
        # Store base URL in state
        await state.update_data(kimi_base_url=base_url.rstrip("/"))
    ...
    async def _handle_kimi_delete_config(self, callback: CallbackQuery, state: FSMContext):
        """Delete Moonshot Kimi configuration"""
        user_id = callback.from_user.id
        lang = await self._get_user_lang(user_id)

        success, message = await self.account_service.clear_kimi_config(user_id)

        if success:
            await self._show_menu(callback, state)
            await safe_callback_answer(callback, "✅ ลบการตั้งค่า Moonshot Kimi แล้ว")
        else:
            await safe_callback_answer(callback, message, show_alert=True)

    async def _handle_reset_to_env(self, callback: CallbackQuery, state: FSMContext):
        """Handle reset to environment defaults"""
        user_id = callback.from_user.id
        lang = await self._get_user_lang(user_id)

        # Confirm with user
        text = (
            f"🔄 <b>รีเซ็ตเป็นค่าเริ่มต้น</b>\n\n"
            f"การดำเนินการนี้จะ:\n"
            f"• ลบ API Key ที่บันทึกไว้ทั้งหมด\n"
            f"• รีเซ็ตการตั้งค่า provider\n"
            f"• ใช้ค่าจากไฟล์ .env แทน\n\n"
            f"<b>ต้องการดำเนินการต่อหรือไม่?</b>"
        )

        await callback.message.edit_text(
            text,
            reply_markup=Keyboards.confirm_reset_to_env(lang=lang),
            parse_mode="HTML"
        )
        await safe_callback_answer(callback)

    async def _confirm_reset_to_env(self, callback: CallbackQuery, state: FSMContext):
        """Confirm reset to environment defaults"""
        user_id = callback.from_user.id
        lang = await self._get_user_lang(user_id)

        success, message, settings = await self.account_service.reset_to_env_defaults(user_id)
        ...
        if success:
            ...
            await safe_callback_answer(callback, "✅ รีเซ็ตสำเร็จ")
        else:
            await safe_callback_answer(callback, message, show_alert=True)
