...
class FileMessageHandler(BaseMessageHandler):
    ...
    async def _handle_file_message(
        ...
        if not user:
            await message.answer(t('error.not_authorized'))
            return

        if self._is_task_running(user_id):
            await message.answer(
                t('error.task_already_running'),
                reply_markup=Keyboards.claude_cancel(user_id)
            )
            return

        if not self.file_processor_service:
            await message.answer(t('error.file_processing_unavailable'))
            return
        ...
        try:
            ...
            await message.answer(t('error.download_failed', error=str(e)))
            return
        ...
        if processed.error:
            await message.answer(t('error.file_processing_failed', error=processed.error))
            return
    ...
    async def handle_media_group(self, messages: List[Message], **kwargs) -> None:
        ...
        user = await self.bot_service.authorize_user(user_id)
        if not user:
            await first_message.answer(t('error.not_authorized'))
            return
        ...
        if not self.file_processor_service:
            await first_message.answer(t('error.file_processing_unavailable'))
            return
        ...
        if not processed_files:
            await first_message.answer(t('error.album_processing_failed'))
            return
