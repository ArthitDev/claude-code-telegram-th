        ...
        **kwargs
        ...
        if self.project_service and self.context_service:
            try:
                ...
                if project:
                    # Use project working_dir but validate it exists
                    # If on Windows and path is Linux format, use Windows path instead
                    project_dir = project.working_dir
                    if os.name == 'nt' and project_dir.startswith('/root'):
                        # Convert /root/projects/name to C:\Users\EPIT-Dev\projects\name
                        project_dir = project_dir.replace('/root', 'C:/Users/EPIT-Dev')
