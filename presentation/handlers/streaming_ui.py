...
# Note: These are in English. For full i18n support, these would need to be
# passed through the translator, but that requires async context which isn't
# available in the dataclass render methods.
TOOL_ACTIONS = {
    "bash": ("Executing", "Done"),
    ...
    "grep": ("Searching code", "Found"),
    "webfetch": ("Loading", "Loaded"),
    "websearch": ("Searching web", "Found"),
    "task": ("Running agent", "Agent completed"),
...
class ToolState:
    ...
    def render(self, translator=None) -> str:
        ...
        # Get action label using translator if available
        if translator:
            if self.status == ToolStatus.PENDING:
                label = f"{translator('tool.waiting_permission')}: `{self.name}`"
            elif self.status == ToolStatus.EXECUTING:
                label = translator(f'tool.{self.name}.executing', default=translator('tool.default.executing'))
            else:  # COMPLETED or ERROR
                label = translator(f'tool.{self.name}.completed', default=translator('tool.default.completed'))
        else:
            # Fallback to TOOL_ACTIONS for backward compatibility
            actions = TOOL_ACTIONS.get(self.name, ("Processing", "Done"))
            if self.status == ToolStatus.PENDING:
                label = f"Waiting: `{self.name}`"
            elif self.status == ToolStatus.EXECUTING:
                label = actions[0]
            else:  # COMPLETED or ERROR
                label = actions[1]
...
class StreamingUIState:
    ...
    completion_status: str = ""  # e.g., "[OK] Done"

    # Whether the message is finalized
    finalized: bool = False

    # Translator for i18n
    _translator: Optional = None
    ...
    def render_non_content(self) -> str:
        ...
        for element in self.elements:
            if element.type == ElementType.CONTENT:
                ...
                # Render tool status with translator
                parts.append(element.data.render(translator=self._translator))
