...
class Keyboards:
    ...
    @staticmethod
    def proxy_settings_menu(has_proxy: bool = False, proxy_status: str = "", lang: str = "en") -> InlineKeyboardMarkup:
        """
    ...
    @staticmethod
    def proxy_type_selection(lang: str = "en") -> InlineKeyboardMarkup:
        """Select proxy type"""
        from shared.i18n import get_translator
        t = get_translator(lang)
    ...
    @staticmethod
    def proxy_auth_options(lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def proxy_scope_selection(lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def proxy_confirm_test(success: bool, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    def language_select(current_lang: str = None) -> InlineKeyboardMarkup:
        ...
        for code, name in languages:
            ...
            text = f"[OK] {name}" if code == current_lang else name
            buttons.append([
    ...
    @staticmethod
    def os_select(current_os: str = None) -> InlineKeyboardMarkup:
        """OS selection keyboard for first launch or settings"""
        os_options = [
            ("windows", "🪟 Windows", "C:\\Users\\<user>\\projects"),
            ("linux", "🐧 Linux (Root)", "/root/projects"),
            ("linux_user", "🐧 Linux (User)", "/home/<user>/projects"),
            ("macos", "🍎 macOS", "/Users/<user>/projects"),
            ("auto", "🔍 Auto Detect", "Detect from system"),
        ]

        buttons = []
        for code, name, path in os_options:
            # Add checkmark if this is the current OS
            text = f"✅ {name}" if code == current_os else name
            buttons.append([
                InlineKeyboardButton(text=text, callback_data=f"os:select:{code}")
            ])

        # Add back button only if we have current OS (i.e., not first launch)
        if current_os:
            buttons.append([
    ...
    def menu_settings(
        ...
        # Auth mode icons
        if auth_mode == "claude_account":
            auth_icon = "☁️"
        elif auth_mode == "moonshot_kimi":
            auth_icon = "🌙"
        elif auth_mode == "zai_api":
            auth_icon = "🌐"
        else:
            auth_icon = "🖥️"
    ...
    @staticmethod
    def menu_system(has_active_task: bool = False, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def menu_help(lang: str = "en") -> InlineKeyboardMarkup:
        """Help submenu"""
        from shared.i18n import get_translator
        t = get_translator(lang)
    ...
    @staticmethod
    def menu_back_only(back_to: str = "menu:main", lang: str = "en") -> InlineKeyboardMarkup:
        """Simple back button keyboard"""
        from shared.i18n import get_translator
        t = get_translator(lang)
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=t("keyboard.back"), callback_data=back_to)]
        ])
    ...
    def main_menu(lang: str = "en") -> ReplyKeyboardMarkup:
        ...
        t = get_translator(lang)
        buttons = [
            [KeyboardButton(text=f"📊 {t('system.metrics')}"), KeyboardButton(text=f"🐳 {t('system.docker')}")],
            [KeyboardButton(text=f"📂 {t('menu.projects')}"), KeyboardButton(text="⚡ YOLO")],
            [KeyboardButton(text=f"🗑️ {t('context.clear')}"), KeyboardButton(text=f"ℹ️ {t('menu.help')}")]
        ]
    ...
    @staticmethod
    def command_approval(command_id: str, command: str, is_dangerous: bool = False, lang: str = "en") -> InlineKeyboardMarkup:
        ...
        buttons = [
            [
                InlineKeyboardButton(text=f"{warning}{t('keyboard.execute')}", callback_data=f"exec:{command_id}"),
                InlineKeyboardButton(text=t("keyboard.cancel"), callback_data=f"cancel:{command_id}")
            ]
        ]
        return InlineKeyboardMarkup(inline_keyboard=buttons)
    ...
    def container_actions(
        ...
        row = []
        if status == "running":
            row.append(InlineKeyboardButton(text=t("keyboard.stop"), callback_data=f"docker:stop:{container_id}"))
            row.append(InlineKeyboardButton(text=t("keyboard.restart"), callback_data=f"docker:restart:{container_id}"))
        else:
            row.append(InlineKeyboardButton(text=t("keyboard.start"), callback_data=f"docker:start:{container_id}"))

        if row:
            buttons.append(row)

        buttons.append([
            InlineKeyboardButton(text=t("keyboard.logs"), callback_data=f"docker:logs:{container_id}"),
            InlineKeyboardButton(text=t("keyboard.remove"), callback_data=f"docker:rm:{container_id}")
        ])

        # Back button
        if show_back:
            buttons.append([
                InlineKeyboardButton(text=t("keyboard.back_to_list"), callback_data=back_to)
            ])

        return InlineKeyboardMarkup(inline_keyboard=buttons)

    @staticmethod
    def session_actions(session_id: str, lang: str = "en") -> InlineKeyboardMarkup:
        """Keyboard for session actions"""
        from shared.i18n import get_translator
        t = get_translator(lang)
        buttons = [
            [
                InlineKeyboardButton(text=t("keyboard.export_md"), callback_data=f"session:export:md:{session_id}"),
                InlineKeyboardButton(text=t("keyboard.export_json"), callback_data=f"session:export:json:{session_id}")
            ],
            [
                InlineKeyboardButton(text=t("keyboard.delete"), callback_data=f"session:delete:{session_id}")
            ]
        ]
        return InlineKeyboardMarkup(inline_keyboard=buttons)

    @staticmethod
    def user_actions(user_id: int, lang: str = "en") -> InlineKeyboardMarkup:
        """Keyboard for user management"""
        from shared.i18n import get_translator
        t = get_translator(lang)
        buttons = [
            [
                InlineKeyboardButton(text=t("keyboard.activate"), callback_data=f"user:activate:{user_id}"),
                InlineKeyboardButton(text=t("keyboard.deactivate"), callback_data=f"user:deactivate:{user_id}")
            ],
            [
                InlineKeyboardButton(text=t("keyboard.set_role"), callback_data=f"user:role:{user_id}")
            ]
        ]
        return InlineKeyboardMarkup(inline_keyboard=buttons)

    @staticmethod
    def role_selection(user_id: int, lang: str = "en") -> InlineKeyboardMarkup:
        """Keyboard for role selection"""
        from shared.i18n import get_translator
        t = get_translator(lang)
        buttons = [
            [
                InlineKeyboardButton(text=t("keyboard.admin"), callback_data=f"role:set:{user_id}:admin"),
                InlineKeyboardButton(text="🔧 DevOps", callback_data=f"role:set:{user_id}:devops")
            ],
            [
                InlineKeyboardButton(text=t("keyboard.user"), callback_data=f"role:set:{user_id}:user"),
                InlineKeyboardButton(text=t("keyboard.readonly"), callback_data=f"role:set:{user_id}:readonly")
            ]
        ]
        return InlineKeyboardMarkup(inline_keyboard=buttons)
    ...
    def system_metrics(
        ...
        buttons = [
            [
                InlineKeyboardButton(text=t("keyboard.refresh"), callback_data="metrics:refresh"),
                InlineKeyboardButton(text=t("keyboard.top_processes"), callback_data="metrics:top")
            ],
            [
                InlineKeyboardButton(text=t("keyboard.containers"), callback_data="docker:list"),
                InlineKeyboardButton(text=t("keyboard.history"), callback_data="commands:history")
            ]
        ]
    ...
    @staticmethod
    def back(button: str = "main", lang: str = "en") -> InlineKeyboardMarkup:
        """Back button"""
        from shared.i18n import get_translator
        t = get_translator(lang)
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=t("keyboard.back"), callback_data=f"back:{button}")]
        ])
    ...
    def docker_list(
        ...
        action_buttons = [
            InlineKeyboardButton(text=t("keyboard.refresh"), callback_data="docker:list")
        ]
        if show_back:
    ...
    @staticmethod
    def claude_permission(user_id: int, tool_name: str, request_id: str, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def claude_question(user_id: int, options: List[str], request_id: str, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def claude_cancel(user_id: int, lang: str = "en") -> InlineKeyboardMarkup:
        """Keyboard to cancel running Claude Code task"""
        from shared.i18n import get_translator
        t = get_translator(lang)
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=t("keyboard.cancel_task"), callback_data=f"claude:cancel:{user_id}")]
        ])

    @staticmethod
    def claude_continue(user_id: int, session_id: str, lang: str = "en") -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=t("keyboard.continue"),
                    callback_data=f"claude:continue:{user_id}:{session_id}"
                ),
                InlineKeyboardButton(
                    text=t("keyboard.new_session"),
    ...
    @staticmethod
    def plan_approval(user_id: int, request_id: str, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def project_selection(projects: List[Dict[str, str]], lang: str = "en") -> InlineKeyboardMarkup:
    ...
    def project_list(
        ...
        if show_create:
            action_row.append(
                InlineKeyboardButton(text=t("keyboard.create"), callback_data="project:create")
            )
        action_row.append(
            InlineKeyboardButton(text=t("keyboard.browse"), callback_data="project:browse")
        )
    ...
    def context_menu(
        ...
        buttons = [
            [
                InlineKeyboardButton(text=t("keyboard.list"), callback_data="ctx:list"),
                InlineKeyboardButton(text=t("keyboard.new"), callback_data="ctx:new")
            ],
            [
                InlineKeyboardButton(text=t("keyboard.clear"), callback_data="ctx:clear"),
            ]
        ]
    ...
    @staticmethod
    def context_clear_confirm(lang: str = "en") -> InlineKeyboardMarkup:
        """Confirmation keyboard for context clearing"""
        from shared.i18n import get_translator
        t = get_translator(lang)
        return InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text=t("keyboard.yes_delete"), callback_data="ctx:clear:confirm"),
                InlineKeyboardButton(text=t("keyboard.no_cancel"), callback_data="ctx:menu")
            ]
        ])
    ...
    def context_list(
        ...
        buttons.append([
            InlineKeyboardButton(text=t("keyboard.new"), callback_data="ctx:new"),
            InlineKeyboardButton(text=t("keyboard.back"), callback_data="ctx:menu")
        ])
    ...
    def folder_browser(
        ...
        """
        Keyboard for browsing folders in projects root.
        ...
        # Use default root path if not provided
        if current_path is None:
            current_path = ProjectPath.ROOT
        ...
        # Back/Up button
        if current_path != ProjectPath.ROOT:
            parent = os.path.dirname(current_path)
            nav_row.append(
                InlineKeyboardButton(text=t("keyboard.up"), callback_data=f"project:browse:{parent}")
            )

        # Refresh button
        nav_row.append(
            InlineKeyboardButton(text=t("keyboard.refresh"), callback_data="project:browse")
        )
        ...
        if current_path == ProjectPath.ROOT:
            buttons.append([
                InlineKeyboardButton(text=t("keyboard.mkdir"), callback_data="project:mkdir")
            ])

        # Back to menu button
        buttons.append([
            InlineKeyboardButton(text=t("keyboard.back"), callback_data="menu:projects")
        ])

        return InlineKeyboardMarkup(inline_keyboard=buttons)

    @staticmethod
    def project_confirm_create(path: str, name: str, lang: str = "en") -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=t("keyboard.create"),
                    callback_data=f"project:confirm:{path[:40]}"
                ),
                InlineKeyboardButton(
                    text=t("keyboard.cancel"),
    ...
    @staticmethod
    def project_delete_confirm(project_id: str, project_name: str, delete_files: bool = False, lang: str = "en") -> InlineKeyboardMarkup:
        """
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    text=t("keyboard.delete_project_only"),
                    callback_data=f"project:delete_confirm:{project_id}:db"
                )
            ],
            [
                InlineKeyboardButton(
                    text=t("keyboard.delete_with_files"),
    ...
    @staticmethod
    def variable_delete_confirm(name: str, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def variable_cancel(lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def variable_skip_description(lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def global_variable_delete_confirm(name: str, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def global_variable_cancel(lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def global_variable_skip_description(lang: str = "en") -> InlineKeyboardMarkup:
    ...
    def plugins_menu(
        ...
        if show_marketplace:
            action_row.append(
                InlineKeyboardButton(text=t("keyboard.marketplace"), callback_data="plugin:marketplace")
            )
        action_row.append(
            InlineKeyboardButton(text=t("keyboard.refresh"), callback_data="plugin:refresh")
        )
        buttons.append(action_row)
        ...
        if show_back:
            ...
            buttons.append([
                InlineKeyboardButton(text=t("keyboard.close"), callback_data="plugin:close")
            ])
    ...
    def plugins_marketplace(
        ...
        # Back button
        buttons.append([
            InlineKeyboardButton(text=t("keyboard.back"), callback_data="plugin:list")
        ])

        return InlineKeyboardMarkup(inline_keyboard=buttons)

    @staticmethod
    def plugin_confirm_action(name: str, action: str, lang: str = "en") -> InlineKeyboardMarkup:
        """Confirmation for plugin enable/disable"""
        from shared.i18n import get_translator
        t = get_translator(lang)
        if action == "enable":
            confirm_text = t("keyboard.enable")
            callback = f"plugin:enable_confirm:{name[:20]}"
        else:
            confirm_text = t("keyboard.disable")
            callback = f"plugin:disable_confirm:{name[:20]}"

        return InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text=confirm_text, callback_data=callback),
                InlineKeyboardButton(text=t("keyboard.cancel"), callback_data="plugin:list")
            ]
        ])
    ...
    @staticmethod
    def account_menu(
        current_mode: str = "moonshot_kimi",
        has_credentials: bool = False,
        subscription_type: str = None,
        current_model: str = None,
        has_zai_key: bool = False,
        has_kimi_config: bool = False,
        ...
        Args:
            current_mode: Current auth mode ("zai_api", "moonshot_kimi", "claude_account", "local_model")
            ...
            has_kimi_config: Whether user has Moonshot Kimi configuration
            show_back: Show back button instead of close button
            back_to: Callback data for back button
            lang: User language code
        """
        ...
        # Moonshot Kimi button
        kimi_emoji = "✅" if current_mode == "moonshot_kimi" else "🌙"
        kimi_indicator = " 🔑" if has_kimi_config else ""
        buttons.append([
            InlineKeyboardButton(
                text=f"{kimi_emoji} Moonshot Kimi{kimi_indicator}",
                callback_data="account:mode:moonshot_kimi"
            )
        ])
        ...
        # Moonshot Kimi config button - only for moonshot_kimi mode
        if current_mode == "moonshot_kimi":
            key_text = "🔑 Setup Kimi API" if not has_kimi_config else "🔑 Change Kimi Config"
            buttons.append([
                InlineKeyboardButton(
                    text=key_text,
                    callback_data="account:kimi_setup"
                )
            ])

        # Reset to environment defaults button
        has_custom_config = has_zai_key or has_kimi_config
        if has_custom_config:
            buttons.append([
                InlineKeyboardButton(
                    text="🔄 Reset to .env defaults",
                    callback_data="account:reset_to_env"
                )
            ])
    ...
    @staticmethod
    def account_auth_options(lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def zai_auth_options(lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def kimi_auth_options(lang: str = "en") -> InlineKeyboardMarkup:
        """Keyboard with options for Moonshot Kimi authorization"""
        from shared.i18n import get_translator
        t = get_translator(lang)
        ...
        return InlineKeyboardMarkup(inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="⚙️ Custom URL + Key",
                    callback_data="account:kimi_setup_url"
                )
            ],
            [
                InlineKeyboardButton(
    ...
    @staticmethod
    def kimi_api_submenu(
        has_config: bool = False,
        current_model: str = None,
        lang: str = "en"
    ) -> InlineKeyboardMarkup:
        """
        Submenu for Moonshot Kimi with config management options.

        Args:
            has_config: Whether user has Kimi config
            current_model: Currently selected model
            lang: User language code
        """
        from shared.i18n import get_translator
        ...
        if has_config:
            ...
            buttons.append([
                InlineKeyboardButton(
                    text=t("menu.delete") + " Config",
                    callback_data="account:kimi_delete"
                )
            ])
        else:
            buttons.append([
    ...
    @staticmethod
    def kimi_api_key_input(has_existing_key: bool = False, lang: str = "en") -> InlineKeyboardMarkup:
        """Keyboard shown when waiting for Kimi API key input"""
        from shared.i18n import get_translator
        t = get_translator(lang)
    ...
    @staticmethod
    def confirm_reset_to_env(lang: str = "en") -> InlineKeyboardMarkup:
        """Keyboard for confirming reset to environment defaults"""
        from shared.i18n import get_translator
        t = get_translator(lang)
        ...
        return InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✅ " + t("menu.confirm"),
                    callback_data="account:confirm_reset_env"
                )
            ],
            [
                InlineKeyboardButton(
    ...
    @staticmethod
    def account_upload_credentials(lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def account_cancel_login(lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def account_confirm_mode_switch(mode: str, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def cancel_only(back_to: str = "account:menu", lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def local_model_skip_name(default_name: str, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    @staticmethod
    def zai_api_key_input(has_existing_key: bool = False, lang: str = "en") -> InlineKeyboardMarkup:
